...
SELECT C.FIRSTNAME, C.LASTNAME, COUNT(I.CUSTOMERID) AS SALES
FROM CHINOOK.CUSTOMER C
INNER JOIN CHINOOK.EMPLOYEE E ON C.FIRSTNAME = E.FIRSTNAME
INNER JOIN CHINOOK.INVOICE I ON C.CUSTOMERID = I.CUSTOMERID
GROUP BY C.FIRSTNAME, C.LASTNAME;


-- Create a query to check the # of invoice for Steve Murray, Robert Brown, or Robert Walter  --This is just to test the above answer
SELECT C.FIRSTNAME, C.LASTNAME, I.CUSTOMERID
FROM CHINOOK.CUSTOMER C
INNER JOIN CHINOOK.INVOICE I ON C.CUSTOMERID = I.CUSTOMERID
WHERE I.CUSTOMERID = 54;
